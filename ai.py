    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r") as f:
                ...
                loaded.setdefault("api_key", DEFAULT_API_KEY)
...
def banner():
    ...
    print(f"{colors.bright_red}WormGPT AnMoL <3{colors.reset}")
    print(f"{colors.bright_cyan}OpenRouter API | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{colors.reset}")
    print(f"{colors.bright_cyan}Made With Love <3 {colors.bright_red}  A N M O L  {colors.reset}- {colors.bright_red}t.me/forever_hidden\n")
...
def select_model():
    ...
    print(f"{colors.yellow}2. Use default (DeepSeek R1 Free){colors.reset}")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 429:
                ...
                print(f"{colors.yellow}Rate limited. Retrying in {retry_after} seconds... (Attempt {attempt + 1}/{max_retries}){colors.reset}")
            ...
            if attempt == max_retries - 1:
                return f"[WormGPT] API Error after {max_retries} attempts: {str(e)}"
            delay = base_delay * (2 ** attempt)  # Exponential backoff
            print(f"{colors.yellow}Request failed. Retrying in {delay} seconds... (Attempt {attempt + 1}/{max_retries}){colors.reset}")
            time.sleep(delay)
        except Exception as e:
            return f"[WormGPT] Unexpected error: {str(e)}"
    
    return f"[WormGPT] Max retries exceeded."
...
def chat_session():
    ...
    while True:
        try:
            ...
            if response.startswith("[WormGPT] API Error"):
